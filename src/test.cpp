...
{	
	...
	hog.detectMultiScale(gray, list, 0, Size(2,2), Size(64,64), 0.8, 2, false);
...
int main(int, char** )
{
	...
	while(true)
	{
		...
		vector<Rect> list = measurement_model(frame1);
	...
	for(;;)
	{
		...
		vector<Rect> list = measurement_model(frame1, fgMaskMOG1);
		...
		list = measurement_model(frame2, fgMaskMOG2);
		...
		list = measurement_model(frame3, fgMaskMOG3);
